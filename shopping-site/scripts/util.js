...
String.prototype.firstWord = function (){ return this.split(" ")[0]
};
var DomUtils = {
        ...
        if (Cart.cartItems.length == 0) {
          $('.main').innerHTML = `
            <div class="cartItems">
              ...
              <a href ="tel:+2349086417512"><button class="bigBtn order"> shop</button>
              </a>
            </div>
            `
        }else{
...
var Cart = {
  ...
  addToCart: function (e){
    ...
    Cart.cartItems.push({"id": key, "qty": 1});
    ...
    if(Cart.cartItems != []){
      for(var i = 0;i < Cart.cartItems.length; i++){
        if(Cart.cartItems[i].id == id){
          found = {where: i, qty: Cart.cartItems[i].qty};
          break;
        }
      }
    }
